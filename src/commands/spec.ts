...
function applyPlaceholders(content: string, name: string): string {
...
export async function specNew(name: string, options?: { template?: string }) {
	...
	try {
		...
		if (template) {
			const specContent = applyPlaceholders(template.spec, name);
			const acceptanceContent = applyPlaceholders(template.acceptance, name);
