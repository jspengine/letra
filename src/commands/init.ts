	...
	return new Promise((resolve) => {
		...
		rl.question(formatted, (answer) => {
			...
			if (trimmed && options.some((o) => o.toLowerCase().startsWith(trimmed))) {
				resolve(
					options.find((o) => o.toLowerCase().startsWith(trimmed)) ??
						options[0],
				);
		...
		switch (tool) {
			case "cursor":
				adapters.push({
					...
					content: baseContent("Cursor", "at"),
				});
				break;
			case "claude-code":
				adapters.push({
					path: join(root, "CLAUDE.md"),
					content: baseContent("Claude Code", "text"),
				});
				break;
			case "windsurf":
				adapters.push({
					path: join(root, ".windsurfrules"),
					content: baseContent("Windsurf", "at"),
				});
				break;
			case "vscode":
				adapters.push({
					path: join(root, ".github", "copilot-instructions.md"),
					content: baseContent("VSCode Copilot", "text"),
				});
				break;
			case "opencode":
				adapters.push({
					path: join(root, "AGENTS.md"),
					content: baseContent("OpenCode", "text"),
				});
				break;
		}
	}
...
export async function init(targetPath?: string, options?: { yes?: boolean }) {
	...
	try {
		...
		if (options?.yes || !process.stdin.isTTY) {
			...
			projectType = await ask("Project type?", [
				"web-app",
				"cli",
				"library",
				"mobile",
