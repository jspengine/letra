...
export default function WorkspaceSetupFlow({ onComplete, onCancel, existingNames }: Props) {
	...
	const governanceOperationCount =
		plan?.operations.filter((operation) => operation.path.endsWith("/workflow.json")).length ??
		0;
	const linkOperationCount =
		plan?.operations.filter((operation) => operation.path.endsWith("/.letra-link")).length ?? 0;
	const adapterOperationCount =
		plan?.operations.filter(
			(operation) =>
				!operation.path.endsWith("/workflow.json") &&
				!operation.path.endsWith("/.letra-link"),
		).length ?? 0;
	...
	function selectedAdapterCount(target: LocationProposal) {
		return target.adapters.filter(
			(adapter) => selectedAdapters[targetAdapterKey(target.id, adapter.tool)],
		).length;
	}

	function locationRole(target: LocationProposal) {
		const isSolutionRoot =
			normalizePath(target.path) === normalizePath(proposal?.workspace.root ?? "");
			...
			<div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[minmax(0,1.12fr)_minmax(340px,0.88fr)]">
				<section className="flex flex-col gap-6">
					<div className="flex items-start justify-between gap-4 sm:grid sm:grid-cols-[1fr_auto]">
						<div>
							...
							>
								Crie a pasta de governança do Letra e vincule as pastas/projetos que
								ela deve acompanhar.
							</p>
						</div>
					...
					<div
						className="grid gap-4 rounded-[var(--radius-md)] border p-4"
						style={{ borderColor: "var(--color-border)" }}
					>
						<div>
							<h2 className="text-lg font-semibold">Workspace e pastas/projetos</h2>
							<p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>
								Informe o nome do workspace e uma pasta de projeto inicial. Nada
								será escrito antes da confirmação final.
							</p>
						</div>
					...
					>
						<div>
							...
							<p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>
								Revise as pastas/projetos vinculadas ao workspace antes de gerar a
								prévia.
							</p>
						</div>
						{proposal ? (
								...
								{proposal.warnings.map((warning) => (
									...
									>
										...
										<span
											...
											style={{ color: "var(--color-text-secondary)" }}
										>
											Adapters disponíveis aparecem nas opções avançadas e só
											serão escritos nas pastas onde forem marcados.
										</span>
									</div>
								))}
										...
										>
											<div className="flex flex-col gap-2">
												<div className="flex items-start justify-between gap-3">
													...
													>
														{selectedAdapterCount(target) > 0
															? `${selectedAdapterCount(target)} adapter(s)`
															: "só .letra-link"}
													</Badge>
												</div>
												...
												>
													...
													>
														Marque adapters apenas nas pastas onde o
														Letra deve criar instruções para agentes. Em
														geral, use a raiz Git/solução.
													</p>
							...
							>
								A proposta aparecerá aqui depois da análise da pasta/projeto
								inicial.
							</div>
						)}
					</div>
					...
					{created && (
						<div
							className="flex flex-col items-center gap-4 rounded-[var(--radius-md)] border p-8 text-center"
							style={{ borderColor: "var(--color-border)" }}
						>
							<Icon
								name="check-circle"
				...
				>
					...
					{proposal && plan ? (
						<>
							<div
								className="grid gap-2 rounded-[var(--radius-sm)] border p-3"
								style={{ borderColor: "var(--color-border)" }}
							>
								<p
								...
								<p className="text-sm">
									{governanceOperationCount || 1} pasta de governança fora do
									projeto, {linkOperationCount} vínculo(s) .letra-link e{" "}
									{adapterOperationCount} arquivo(s) de adapter.
								</p>
								<p
									className="text-xs"
									style={{ color: "var(--color-text-secondary)" }}
								>
									Adapters serão escritos em{" "}
									{selectedAdapterLocations.length > 0
										? selectedAdapterLocations
												.map((loc) => loc.label)
												.join(", ")
										: "nenhuma pasta"}
									.
							...
							<div className="grid gap-3">
								...
								>
									...
									<p className="mt-1 text-sm">
										Criado em {dataDir}/workflow.json
									</p>
								</div>
								<div
									className="rounded-[var(--radius-sm)] border p-3"
								...
								>
									...
									<p className="mt-1 text-sm">
										Cada pasta/projeto recebe apenas .letra-link apontando para
										o data directory.
									</p>
								</div>
								<div
							...
							<div className="flex flex-col gap-2">
								{plan.operations.map((operation) => (
									<div
										key={`${operation.kind}:${operation.path}:${operation.tool || ""}`}
										className="rounded-[var(--radius-sm)] border p-3"
										style={{ borderColor: "var(--color-border)" }}
									>
										<div className="flex items-start justify-between gap-3">
						...
						>
							Gere a prévia para ver exatamente quais arquivos serão criados,
							preservados ou bloqueados por conflito.
						</div>
					)}
					<Button
						type="button"
					...
					{proposal && (
						<div
							className="mt-2 rounded-[var(--radius-sm)] border p-3"
							style={{ borderColor: "var(--color-border)" }}
						>
							<p
								className="text-xs font-semibold uppercase"
							...
							<p className="mt-1 truncate font-mono text-xs">
								{proposal.workspace.root}
							</p>
							<p className="mt-2 text-sm">
								{proposalLocations.length} pasta(s)/projeto(s) detectado(s)
							</p>
							<p className="text-sm">
								{selectedTools.length} adapter(s): {selectedTools.join(", ")}
							</p>
						</div>
					)}
				</aside>
