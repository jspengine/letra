...
function InfoIcon({ tip }: { tip: string }) {
	return (
		<span className="group relative inline-flex items-center">
			...
			/>
			<div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 hidden group-hover:block z-50 w-48 px-2 py-1.5 text-xs text-left rounded shadow-lg pointer-events-none" style={{ background: "var(--card)", color: "var(--card-foreground)", border: "1px solid var(--border)" }}>
				{tip}
			</div>
		</span>
	);
...
export default function HomeView({ workflow, onSelectItem, onTabChange }: Props) {
	...
	useEffect(() => {
		fetch("/api/specs")
			.then((r) => r.json())
			.then((data) => { if (Array.isArray(data)) setSpecs(data); })
			.catch(() => {});
		fetch("/api/focus")
			.then((r) => r.json())
	...
	const doingItems = workflow.items.filter((it) => {
		...
		return st?.zone === "doing" || (!st?.zone && workflow.stages.indexOf(st!) > 0 && workflow.stages.indexOf(st!) < workflow.stages.length - 1);
	}).length;
	const doneItems = workflow.items.filter((it) => {
		const st = workflow.stages.find((s) => s.id === it.stage);
		return st?.zone === "done" || (!st?.zone && workflow.stages.indexOf(st!) === workflow.stages.length - 1);
	}).length;
	const staleItems = workflow.items.filter((it) => daysSince(it.createdAt) > 7).length;

	const specValid = specs.filter((s) => /## Outcome/.test(s.content) && /## Constraints/.test(s.content) && /## Acceptance Criteria/.test(s.content)).length;
	const specNoDate = specs.filter((s) => !/> Updated:\s*\d{4}-\d{2}-\d{2}/.test(s.content)).length;
				...
				<div className="flex flex-col gap-6">
					...
					<div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
						...
						<Card className="p-4 flex flex-col items-center text-center gap-1 transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5 border-muted/60">
							...
							<span className="text-2xl font-bold" style={{ color: specDrift > 0 ? "var(--warning)" : specNoDate > 0 ? "var(--muted-foreground)" : "var(--success)" }}>
								{specDrift > 0 ? specDrift : specNoDate > 0 ? `${specNoDate}?` : "0"}
							</span>
							<span className="text-xs" style={{ color: "var(--muted-foreground)" }}>
								{specDrift > 0 ? `${specDrift} desatualizadas há 7d+` : specNoDate > 0 ? `${specNoDate} sem data` : "todas atualizadas"}
							</span>
						</Card>
						<Card className="p-4 flex flex-col items-center text-center gap-1 transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5 border-muted/60">
							<span className="text-xs font-medium uppercase tracking-wider flex items-center gap-1.5" style={{ color: "var(--muted-foreground)" }}>
							...
							{focus?.active ? (
								<span className="text-lg font-semibold truncate max-w-full">{focus.spec}</span>
							) : (
								<span className="text-2xl font-bold" style={{ color: "var(--muted-foreground)" }}>—</span>
							)}
							<span className="text-xs" style={{ color: "var(--muted-foreground)" }}>
								{focus?.active ? focus.content?.split("\n").find((l) => l.includes("**Outcome**"))?.replace(/\*\*/g, "").replace("Outcome:", "").trim() || "Em foco" : "nenhum foco definido"}
							</span>
						</Card>
						<Card className="p-4 flex flex-col items-center text-center gap-1 transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5 border-muted/60">
							<span className="text-xs font-medium uppercase tracking-wider flex items-center gap-1.5" style={{ color: "var(--muted-foreground)" }}>
					...
					<div className="flex flex-col gap-2">
						<h2 className="text-sm font-semibold flex items-center gap-2">
							...
							<button onClick={() => onTabChange?.("flow")} className="text-xs text-primary hover:underline font-normal ml-auto">
								[ver fluxo completo]
							</button>
						</h2>
						<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
							{localStages.map((stage, i) => {
								...
								const isDoing = stage.zone === "doing" || (i > 0 && i < workflow.stages.length - 1);
								...
								return (
									<Card
										...
										style={stage.color ? { borderTop: `2px solid ${stage.color}80` } : undefined}
									>
										<CardContent className="p-0 flex flex-col gap-2">
											<div className="flex items-center gap-1.5 text-xs">
												<span className="w-2 h-2 rounded-full shrink-0" style={{
													background: stage.color ?? (stage.zone === "todo" ? "var(--primary)" : stage.zone === "done" ? "var(--success)" : "var(--warning)")
												}} />
												{stage.color && <span className="w-3 h-0.5 rounded shrink-0" style={{ background: `${stage.color}40` }} />}
												<span className="truncate font-medium">{stage.name}</span>
												<span className="font-bold ml-auto">{stageItems.length}</span>
											</div>
											<div className="flex flex-col gap-1 min-h-[24px]">
												{miniItems.length === 0 && (
													<div className="flex items-center justify-center h-6 rounded border border-dashed" style={{ borderColor: "var(--border)" }}>
													...
													return (
														<div
															...
															className={cn(
																...
																stage.color ? "border-transparent" : "border-border/50",
															)}
															style={{
																background: stage.color ? `${stage.color}12` : "var(--muted)",
																borderLeft: stage.color ? `2px solid ${stage.color}60` : undefined,
															}}
														>
															<Icon name="list-three" size={10} className="opacity-30 shrink-0" />
															<span className="truncate flex-1">{it.id}</span>
															<span className="shrink-0 tabular-nums" style={{ color: days <= 2 ? "var(--muted-foreground)" : days <= 7 ? "var(--warning)" : "var(--error)" }}>
																{days}d
															</span>
														</div>
													);
												...
												{stageItems.length > 3 && (
													<button onClick={() => onTabChange?.("flow")} className="text-[10px] text-primary hover:underline text-left">
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-6">
						<div className="lg:col-span-2 xl:col-span-2 flex flex-col gap-4">
							<div className="flex flex-col gap-2">
								<div className="flex items-center justify-between">
									...
									<button onClick={() => onTabChange?.("specs")} className="text-xs text-primary hover:underline">
										[ver todas]
									</button>
								</div>
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
									{specs.slice(0, 4).map((spec) => {
										...
										return (
											<Card key={spec.id} className="p-3 transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5 border-muted/60">
												<CardContent className="p-0">
													<div className="flex items-center gap-2">
														<span className="text-sm font-medium truncate flex-1">{spec.id}</span>
														<Badge variant={hasOutcome && hasAC ? "success" : "warning"} className="shrink-0">
															{pct}%
														</Badge>
													</div>
													<div className="flex items-center gap-2 mt-1 text-xs" style={{ color: "var(--muted-foreground)" }}>
														<span className="flex items-center gap-1">
															<Icon name="check" size={14} style={{ color: hasOutcome && hasAC ? "var(--success)" : "var(--warning)" }} />
															{acTotal} ACs
														</span>
						...
						<div className="flex flex-col gap-4">
							<div className="flex flex-col gap-2">
								...
								{recentDecisions.length === 0 ? (
									...
									<div className="flex flex-col gap-2">
										{recentDecisions.map((d) => (
											<Card key={d.name} className="p-3 transition-all duration-200 hover:shadow-sm hover:-translate-y-0.5 border-muted/60">
												<CardContent className="p-0">
													<div className="text-xs" style={{ color: "var(--muted-foreground)" }}>
														{formatDate(d.name)}
													</div>
													<div className="text-sm font-medium truncate">{resolveTitle(d.content) || d.name}</div>
												</CardContent>
											</Card>
										))}
									</div>
								...
								{decisions.length > 4 && (
									<button className="text-xs text-primary hover:underline self-start">+{decisions.length - 4} mais</button>
								)}
							</div>
						</div>
							...
							<Card className="p-3 transition-all duration-200 hover:shadow-sm border-muted/60">
								<CardContent className="p-0 flex flex-col gap-1">
									{workflow.stages.map((stage) => {
										const items = workflow.items.filter((it) => it.stage === stage.id);
										const avg = items.length > 0
											? Math.round(items.reduce((acc, it) => acc + daysSince(it.createdAt), 0) / items.length * 10) / 10
											: 0;
										const max = items.length > 0
											? Math.max(...items.map((it) => daysSince(it.createdAt)))
											: 0;
										return (
											<div key={stage.id} className="flex items-center gap-2 text-xs py-1 border-b last:border-0 transition-colors duration-150 hover:bg-primary/5" style={{ borderColor: "var(--border)" }}>
