import type { HTMLAttributes } from "react";
import { cn } from "./utils";

interface SkeletonProps extends HTMLAttributes<HTMLDivElement> {}

export function Skeleton({ className, ...props }: SkeletonProps) {
	return (
		<div
			className={cn("skeleton-shimmer rounded-[var(--radius-sm)]", className)}
			{...props}
		/>
	);
}

export function SkeletonCard() {
	return (
		<div
			className="rounded-[var(--radius-md)] border-[length:var(--border-thin)] p-[var(--space-4)] flex flex-col gap-[var(--space-3)]"
			style={{ background: "var(--card)", borderColor: "var(--border)" }}
		>
			<Skeleton className="h-4 w-1/3" />
			<Skeleton className="h-3 w-full" />
			<Skeleton className="h-3 w-2/3" />
		</div>
	);
}

export function SkeletonPipeline() {
	return (
		<div className="flex flex-col gap-[var(--space-4)] p-[var(--space-6)] max-w-3xl mx-auto w-full">
			<Skeleton className="h-6 w-48" />
			<Skeleton className="h-3 w-32" />
			{[1, 2, 3, 4, 5].map((i) => (
				<div key={i} className="flex gap-[var(--space-3)]">
					<Skeleton className="w-10 h-10 rounded-full shrink-0" />
					<div className="flex-1">
						<Skeleton className="h-24 w-full rounded-[var(--radius-md)]" />
					</div>
				</div>
			))}
		</div>
	);
}

export function SkeletonTable() {
	return (
		<div className="flex flex-col gap-[var(--space-3)]">
			<Skeleton className="h-8 w-full rounded-[var(--radius-md)]" />
			{[1, 2, 3, 4, 5].map((i) => (
				<Skeleton key={i} className="h-10 w-full rounded-[var(--radius-md)]" />
			))}
		</div>
	);
}

export function SkeletonAgentList() {
	return (
		<div className="flex flex-col gap-[var(--space-3)]">
			<Skeleton className="h-6 w-32" />
			<Skeleton className="h-3 w-48" />
			{[1, 2, 3].map((i) => (
				<Skeleton key={i} className="h-24 w-full rounded-[var(--radius-md)]" />
			))}
		</div>
	);
}

export function SkeletonKanban() {
	return (
		<div className="flex flex-col gap-[var(--space-4)]">
			<div className="grid grid-cols-4 gap-[var(--space-4)]">
				{[1, 2, 3, 4].map((i) => (
					<div key={i} className="flex flex-col gap-[var(--space-2)]">
						<Skeleton className="h-5 w-24" />
						<Skeleton className="h-32 w-full rounded-[var(--radius-md)]" />
					</div>
				))}
			</div>
			<div className="grid grid-cols-5 gap-[var(--space-4)]">
				{[1, 2, 3, 4, 5].map((i) => (
					<div key={i} className="flex flex-col gap-[var(--space-2)]">
						<Skeleton className="h-5 w-24" />
						<Skeleton className="h-32 w-full rounded-[var(--radius-md)]" />
					</div>
				))}
			</div>
		</div>
	);
}
