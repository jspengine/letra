...
export function AvatarWithStatus({
	...
	const diameter =
		size === "lg" ? "var(--icon-lg)" : size === "sm" ? "var(--icon-sm)" : "var(--icon-md)";
	const statusColor =
		status === "online"
			? "var(--color-success)"
			: status === "busy"
	...
	return (
		<div className={cn("relative inline-flex shrink-0", className)} {...props}>
			<div
				...
				style={{
					borderColor:
						status === "thinking" ? "var(--color-agent)" : "var(--color-border)",
			...
			<span
				className={cn("absolute bottom-0 right-0 rounded-full border-2", statusClassName)}
				style={{
					width: diameter,
					height: diameter,
