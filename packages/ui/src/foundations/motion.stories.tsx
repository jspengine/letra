...
const Token = ({ token, value, label }: { token: string; value: string; label: string }) => (
	<div className="flex items-center gap-[var(--space-3)] py-[var(--space-1)]">
		...
		<code className="ml-[var(--space-2)] text-[12px] text-[var(--color-text-disabled)]">
			{value}
		</code>
	</div>
);
...
const Box = ({
	...
	tone?: "primary" | "agent" | "danger" | "success";
}) => (
	...
	render: () => (
		<div style={wrap}>
			...
			<div className="grid grid-cols-4 gap-[var(--space-6)] py-[var(--space-4)]">
				<Box
					label="agent-running"
					animationClassName="animate-agent-running"
					tone="agent"
				/>
				<Box
					label="agent-breathe"
					animationClassName="animate-agent-breathe"
					tone="agent"
				/>
				<Box label="pulse-gate-waiting" animationClassName="animate-pulse-gate-waiting" />
				<Box
					label="pulse-gate-urgent"
					animationClassName="animate-pulse-gate-urgent"
					tone="danger"
				/>
			...
			<p className="m-0 text-[13px] text-[var(--color-text-secondary)]">
				Animações respeitam <code>prefers-reduced-motion</code>. Valide no app com a
				acessibilidade do SO ativada.
			</p>
		</div>
	),
};
