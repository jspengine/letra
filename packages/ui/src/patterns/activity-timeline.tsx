...
export function TimelineItem({ title, description, timestamp, status = "default", icon, action, className, last = false }: TimelineItemProps) {
	const colors = statusStyles(status);
	return (
		<div className={cn("flex gap-[var(--space-3)]", className)}>
			<div className="flex flex-col items-center">
				<div className={cn("mt-1 size-[var(--icon-xs)] shrink-0 rounded-full", colors.dot)} aria-hidden="true" />
				{!last && <div className={cn("w-px flex-1 my-[var(--space-2)]", colors.line)} aria-hidden="true" />}
			</div>
				...
				<div className="flex flex-col gap-[var(--space-3)] sm:flex-row sm:items-start sm:justify-between sm:gap-[var(--space-2)]">
					<div className="min-w-0">
						<div className="flex items-center gap-[var(--space-2)]">
							...
							<span className="min-w-0 text-body font-medium leading-snug" style={{ color: "var(--color-text-primary)" }}>{title}</span>
						</div>
						{description && (
							<p className="mt-[var(--space-1)] text-caption leading-relaxed" style={{ color: "var(--color-text-secondary)" }}>{description}</p>
						)}
					</div>
					<div className="flex shrink-0 flex-wrap items-center gap-[var(--space-2)] sm:justify-end">
						{timestamp && (
							<span className="whitespace-nowrap text-caption tabular-nums" style={{ color: "var(--color-text-secondary)" }}>{timestamp}</span>
						)}
...
export function TimelineSeparator({ className }: { className?: string }) {
	return <div className={cn("w-px flex-1 my-[var(--space-2)] bg-[var(--color-border)]", className)} aria-hidden="true" />;
}
