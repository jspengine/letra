...
`;
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(tmpDir, ".letra", "workflow.json"), JSON.stringify({
      name: "Test",
  ...
  it("includes Direcao do Harness section in text format", () => {
    ...
    const content = formatAdapterContent(snapshot, "text", { source: "flow-ac", displayName: "Test" });
  ...
  it("includes section in @ format for Cursor/Windsurf", () => {
    const snapshot = buildHarnessSnapshot(tmpDir, buildOptions());
    const content = formatAdapterContent(snapshot, "at", { source: "flow-ac", displayName: "Test" });
    expect(content).toContain("## Direção do Harness");
  ...
  it("omits section when there is no workflow", () => {
    ...
    const content = formatAdapterContent(snapshot, "text", { source: "init", displayName: "Test" });
  ...
  it("shows fallback when no item is active", () => {
    const snapshot = buildHarnessSnapshot(tmpDir, buildOptions({ primaryItemId: undefined, workflow: {
      name: "Test",
  ...
  it("shows note when stage has no activity configured", () => {
    const snapshot = buildHarnessSnapshot(tmpDir, buildOptions({ activeStageId: "empty-stage", primaryItemId: undefined, workflow: {
      name: "Test",
      stages: [{ id: "empty-stage", name: "Empty", order: 0 }],
      items: [{ id: "ITEM-2", description: "No Activity Item", stage: "empty-stage" }],
    }}));
    const content = formatAdapterContent(snapshot, "text", { source: "flow-ac", displayName: "Test" });
  ...
  it("resolves <ITEM-ID> in commands", () => {
    ...
    mkdirSync(specDir, { recursive: true });
    writeFileSync(join(specDir, "spec.md"), `# Spec\n## Acceptance Criteria\n- [x] **AC1**: Done\n`);
    const snapshot = buildHarnessSnapshot(tmpDir, buildOptions({ activeStageId: "review", primaryItemId: "ITEM-1", workflow: {
      name: "Test",
      stages: [
      ...
      items: [
        { id: "ITEM-1", description: "Test Item", stage: "review", spec: "test-spec" },
      ],
    }}));
    const content = formatAdapterContent(snapshot, "text", { source: "flow-ac", displayName: "Test" });
  ...
  it("omits <AC-ID> command when no pending ACs exist", () => {
    ...
    const snapshot = buildHarnessSnapshot(tmpDir, buildOptions());
    const content = formatAdapterContent(snapshot, "text", { source: "flow-ac", displayName: "Test" });
    const dirSection = content.substring(content.indexOf("harness-direction:start"), content.indexOf("harness-direction:end"));
  ...
  it("shows gate sub-section when gate activity has unique objective", () => {
    ...
    mkdirSync(specDir, { recursive: true });
    writeFileSync(join(specDir, "spec.md"), `# Spec\n## Acceptance Criteria\n- [x] **AC1**: Done\n`);
    const snapshot = buildHarnessSnapshot(tmpDir, buildOptions({ activeStageId: "design", workflow: {
      name: "Test",
      stages: [
        { id: "design", name: "Design", order: 0 },
        { id: "code", name: "Code", order: 1 },
      ],
      items: [{ id: "ITEM-1", description: "Test Item", stage: "design", spec: "test-spec" }],
    }}));
    const content = formatAdapterContent(snapshot, "text", { source: "flow-ac", displayName: "Test" });
  ...
  it("hides gate sub-section when gate has no unique data vs work activities", () => {
    ...
    const dirSection = content.includes("harness-direction:start")
      ? content.substring(content.indexOf("harness-direction:start"), content.indexOf("harness-direction:end"))
      : "";
  ...
  it("resolves ITEM-ID fallback to items[0] when primaryItemId is null", () => {
    ...
    const snapshot = buildHarnessSnapshot(tmpDir, buildOptions({
      ...
      workflow: {
        ...
        items: [{ id: "ITEM-FALLBACK", description: "Fallback Item", stage: "review", spec: "test-spec" }],
      },
    }));
    const content = formatAdapterContent(snapshot, "text", { source: "flow-ac", displayName: "Test" });
  ...
  it("produces valid harness-direction for all adapter sources", () => {
    ...
    for (const adapter of adapters) {
      const snapshot = buildHarnessSnapshot(tmpDir, buildOptions());
      const content = formatAdapterContent(snapshot, "text", { source: adapter.source as any, displayName: adapter.displayName });
      expect(content).toContain("## Direção do Harness");
      expect(content).toContain("harness-direction:start");
      expect(content).toContain("harness-direction:end");
    }
  ...
  it("updates direction when item moves to a different stage", () => {
    ...
    const snapshotCode = buildHarnessSnapshot(tmpDir, buildOptions({ activeStageId: "code", primaryItemId: "ITEM-1", workflow: {
      name: "Test",
      stages: [
        { id: "code", name: "Code", order: 0 },
        { id: "review", name: "Review", order: 1 },
    ...
    const contentCode = formatAdapterContent(snapshotCode, "text", { source: "flow-ac", displayName: "Test" });
    ...
    const snapshotReview = buildHarnessSnapshot(tmpDir, buildOptions({ activeStageId: "review", primaryItemId: "ITEM-1", workflow: {
      name: "Test",
      stages: [
        { id: "code", name: "Code", order: 0 },
        { id: "review", name: "Review", order: 1 },
      ],
      items: [{ id: "ITEM-1", description: "Test Item", stage: "review", spec: "test-spec" }],
    }}));
    const contentReview = formatAdapterContent(snapshotReview, "text", { source: "flow-ac", displayName: "Test" });
