...
function createTestDir(): string {
	const dir = join(tmpdir(), `letra-activity-context-cmd-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	mkdirSync(join(dir, ".letra", "specs"), { recursive: true });
	return dir;
}

function writeWorkflow(dir: string) {
	writeFileSync(join(dir, ".letra", "workflow.json"), JSON.stringify({
		version: "1.0",
		name: "letra-test",
...
describe("activity-context command", () => {
	...
	it("prints json when requested", async () => {
		...
		await command.parseAsync(["node", "activity-context", "--format", "json", "--activity", "design"], { from: "user" });
