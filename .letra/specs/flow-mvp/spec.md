...
- [ ] **`flow init --quick`**: Wizard com 3 perguntas (nome, estágios, agentes) gera `.letra/flow/v1.0.0.json` válido.
- [ ] **`flow init` (sem `--quick`)**: Wizard completo com perguntas adicionais (artefatos, regras).
- [ ] **`flow board`**: Exibe tabela com todos os estágios, quantidade de itens por estágio, e itens ativos.
- [ ] **`flow move <id> --to <stage>`**: Move item entre estágios e regenera arquivos de adapter.
- [ ] **`flow backlog add <desc>`**: Adiciona item ao backlog no primeiro estágio.
- [ ] **`flow backlog list`**: Lista todos os itens com estágio atual e agente designado.
...
- [ ] **Adapter integrado**: `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.windsurfrules`, `.github/copilot-instructions.md` incluem estágio atual e regras aplicáveis após `flow move`.
