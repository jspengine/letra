...
Quando um agente IA pega uma atividade para executar, o item fica visualmente marcado como "em andamento" em todos os touchpoints (board, dashboard, CLI), sem impedir que o usuário o mova manualmente. O usuário vê de relance qual item está sendo trabalhado e pode decidir se interfere ou não. Não há lock — há sinalização.

## Constraints

- Zero lock: usuário SEMPRE pode mover um item no kanban, mesmo que claimed
- Claim é soft — não impede o agente de continuar trabalhando se o item foi movido
- `claimedBy` e `claimedAt` são salvos em `workflow.json` (item-level), persistidos via `writeWorkflow()`
- TODOS os touchpoints refletem o mesmo estado: web UI (board + dashboard), CLI (flow board, pulse, backlog list), APIs
- Schema compatível com workflow.json existente — campos opcionais, sem quebra
